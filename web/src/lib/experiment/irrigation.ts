...
// Wall-clock time with seconds, for live "now" readouts.
...
export function normalizeWeightLogs(value: unknown): IrrigationWeightLog[] {
  const parsed = z
    .array(weightLogSchema)
    .safeParse(Array.isArray(value) ? value : []);
...
export function toSupabaseIrrigationPayload(
  ...
  return {
    ...
    ...(input.waterTempC !== undefined
      ? { water_temp_c: input.waterTempC }
      : {}),
    ...(input.weightLogs !== undefined
      ? { weight_logs: input.weightLogs }
      : {}),
...
export function formatManilaClock(value: string | Date) {
...
export function expectedWeightSlots(
  ...
  // Exact 10-minute steps from the watering time, strictly before the 6 PM
  // cutoff. A slot landing on or after 6 PM is not considered — at 6 PM the
  // weigh window closes, so such a slot could never be weighed.
...
// View model for the "Next Watering" countdown. Skips any due (overdue)
// checkpoint and targets the next scheduled slot, falling back to the 6 PM
// cutoff. Idle when no weigh window is open.
export function computeNextWatering(
  events: IrrigationEvent[],
  now = new Date(),
): NextWatering {
  const status = computeWateringStatus(events, now);
  ...
  const nextAt = status.nextSlotAt ?? status.cutoffAt;
  return {
    state: "counting",
    nowAt: now.toISOString(),
    nextAt,
    remainingMs: Math.max(0, Date.parse(nextAt) - now.getTime()),
  };
}
...
export function computeDailyWaterUse(
  events: IrrigationEvent[],
): DailyWaterUse[] {
  return activeIrrigationEvents(events)
    ...
    .toSorted(
      (a, b) => Date.parse(a.fromWateredAt) - Date.parse(b.fromWateredAt),
    );
}
